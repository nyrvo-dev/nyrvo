...
func TestServiceMissing(t *testing.T) {
	...
	f := got[0]
	if f.Rule != ServiceMissing || f.Severity != finding.SeverityMedium ||
...
func TestServiceImageMismatch(t *testing.T) {
	ci := serviceSnapshot("test job", snapshot.SourceGitHubActions, "postgres:16")
	...
	f := got[0]
	if f.Rule != ServiceImageMismatch || f.Severity != finding.SeverityMedium ||
...
func TestServiceMissingRecommendationAcknowledgesNativeInstall(t *testing.T) {
	...
	if len(got) != 1 || got[0].Rule != ServiceMissing {
